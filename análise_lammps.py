...
IT=2392000 #Quantidade total de iterações feitas no LAMMPS através do comando run
R_1_O=104000 #Iterações para um dado timestep em que se obtém uma recorrência de primeira ordem
ts=0.1 #Timestep utilizado no script do LAMMPS
...
with open('res.comp') as f: 
    ...
    for line in f:                
        ...
        if ((k-1)%(N+11)!=0 and (k-2)%(N+11)!=0 and (k-3)%(N+11)!=0 and (k-4)%(N+11)!=0 and (k-5)%(N+11)!=0 
            and (k-6)%(N+11)!=0 and (k-7)%(N+11)!=0 and (k-8)%(N+11)!=0 and (k-9)%(N+11)!=0  #O arquivo .comp tem linhas que se repetem, para situar qual iteração entre outras informações
